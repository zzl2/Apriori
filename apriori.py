    ...
    for transaction in dataSet:
        for item in transaction:
            if not [item] in C1:
                C1.append([item])
    C1.sort()
    return list(map(frozenset,C1))    #map将传入的函数依次作用到序列的每个元素，并把结果作为新的list返回。
                                #frozenset() 返回一个冻结的集合，冻结后集合不能再添加或删除任何元素。
def scanD(D,Ck,minSupport):   #数据集  候选项集列表  感兴趣集最小支持度
    ssCnt={}
    for tid in D:
        for can in Ck:
            if can.issubset(tid):    #s1.issubset(s2)  :测试是否 s1 中的每一个元素都在 s2 中,运算符操作为 s1<=s2;
                if not can in ssCnt :
                    ssCnt[can]=1
                else : ssCnt[can]+=1
